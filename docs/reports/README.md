...
There are fourteen campaigns. They answer twenty-one questions, in this order — phase 4
answers eight of them, because whether a benchmark can read damage, whether the model has
...
published, whether the model it scores is the one a person would download, and what the
margin it finally reports is a difference *in* are eight separate failures:
...
| 21 | Is the 4-bit margin a difference in method, or in how closely each arm tracks the ceiling? | **Fidelity, and one identity produces both of its signs** — split by source the +0.64 over GPTQ is +1.03 on wikisql and -0.49 on gretel, which Cochran's Q calls heterogeneous; but the bf16 ceiling is 88.59% on wikisql against 71.63% on gretel, so on this panel *source* and *hard* are the same column and a source-wise test cannot tell them apart. Stratify instead by whether the **ceiling** got the item right — a label owing nothing to either compared arm, so McNemar stays valid inside each half — and the margin does not merely vary, it **changes sign with difficulty**: **+1.18** over 10 111 items (*p*=1.38e-06) against **-2.22** over 1 889 (*p*=0.00966), and it changes sign *within wikisql alone*, +1.58 against -3.24, which the source axis cannot produce. The account is one measurement. A hit is a boolean, so `accuracy = c·f_right + (1-c)·(1-f_wrong)` is an **identity**, and DynQuant agrees with the ceiling on **95.25%** of items against GPTQ's 93.91% (**+1.34**, *p*=3.93e-08) while the two baselines do not separate from each other on fidelity at all (+0.35, *p*=0.198) any more than on accuracy (*p*=0.272). An arm that tracks the ceiling more closely inherits its right answers *and* its wrong ones, so it **must** win the 84% of items where the ceiling is right and lose the other 16%: the second stratum is the first one's fidelity delta with its sign flipped, identical *p* and the two discordant counts swapped. “The margin changes sign with difficulty” is that identity, not a second finding. It also prices the source variation without appealing to the sources, since a point of fidelity moves accuracy by `2c-1` — 0.77 on wikisql, 0.43 on gretel. The control is the row worth reading twice: GPTQ and AWQ separate on neither aggregate and their margin still moves across the four crossed cells (Q=8.73), so **cell structure is a property this panel finds in the baselines** and a heterogeneous row is a statement about the mixture rather than a signature of the method under test. The same instrument reads the 3-bit collapse from the other end — `gptq_3b` agrees with bf16 on **87.08%** of the items bf16 got *wrong* and **69.70%** of the ones it got right, an arm that has stopped tracking the ceiling and “agrees” on the failures by being broadly wrong — and the identity returns its 60.76% exactly. What none of this settles is **whose** fidelity: the DynQuant arms ran `grouped_mm` and the linearised baselines ran `eager`, and §8 measured those two dispatches disagreeing on **1.24%** of teacher-forced tokens, **0.29x** the 1.34-point gap being reported and on the same axis. The `--experts-impl eager` re-score decides it; until it runs, every row here is a margin plus a confound a quarter its size. Both the statistics and the markdown are generated: `panel_table.py` cuts the same six comparisons by source, by the ceiling's own answer and by both crossed, and `report_tables.py` formats the payload it writes — carrying the confound flag and the size of the Holm family onto every row, because a table pasted without its qualifications is a table that lost them | [`phase4-text2sql-mixture.md`](phase4-text2sql-mixture.md) §13 |
...
## 21. Phase 4 — the margin is fidelity, and the confound is a quarter of it

[`phase4-text2sql-mixture.md`](phase4-text2sql-mixture.md) §13 · measured 2026-08-10

The panel's headline is DynQuant +0.64 points over GPTQ at 4 bits, and that number is not one
number. By source it is +1.03 on wikisql and -0.49 on gretel. But the two sources differ in more
than identity — the bf16 ceiling is 88.59% on one and 71.63% on the other — so a source-wise
...
every item, so no `sources.json` is needed. The margin changes sign: +1.18 where bf16 is right,
-2.22 where it is wrong, and it does the same *within wikisql alone*.
...
DynQuant agrees with bf16 on 95.25% of items against GPTQ's 93.91%; the two baselines do not
...
The difficulty split is that identity being read, not independent evidence for it.

**The baselines carry cell structure too.** Crossing source with difficulty, GPTQ against AWQ —
which separate on neither aggregate — still moves across the four cells at Q=8.73. So a
heterogeneous row says which part of a mixture a margin comes from; it is not a property of the
method under test.

**And the whole section is one confound wide.** The DynQuant arms ran `grouped_mm` while the
linearised baselines ran `eager`, and §8 measured those dispatches disagreeing on 1.24% of
teacher-forced tokens — 0.29x the fidelity gap, on the same axis. Nothing here separates method
from dispatch, and the `--experts-impl eager` re-score is the only thing that will.

The statistics and the tables are both generated now. `panel_table.py` cuts the six comparisons
three ways and carries all three into the json the model cards read; `report_tables.py` formats
that payload into the markdown this section prints, so the rows arrive with the confound flag and
the Holm family size attached rather than losing them somewhere between a terminal and a report.
