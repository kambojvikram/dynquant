...
| 22 | At 3 bits DynQuant beats GPTQ by nineteen points — is that the signal, or the shape of the map? | **Unmeasured here, and the row says so rather than banking it.** At 3 332 904 576 B — 5.08x compression, every arm within 0.05% of the anchor — GPTQ scores 60.76% and AWQ 57.92% against a bf16 ceiling of 84.29%; DynQuant scores **79.89%**, **+19.13** over GPTQ (+2781/-485) and **+21.98** over AWQ (+3150/-513), both at *p* below double precision, and -4.40 from the ceiling it was encoded from. It is the only 3-bit arm still emitting SQL: 213 unparseable generations against 1 008 and 1 523. The fidelity instrument reads the difference as a kind rather than a degree — `gptq_3b` agrees with bf16 on **86.68%** of the items bf16 got *wrong* and only **69.60%** of the ones it got right, where-wrong above where-right, which no arm that is still tracking a ceiling can produce, while `dq_3b` keeps the normal ordering at 91.40 against 81.86. **What the map did is legible and generated, not transcribed.** `map_roles.py` reads the exported allocation: all **22 routers held at 8 bits at both budgets** — the widest width in the table, on the smallest tensors, while the budget tightens a full bit per parameter around them — paid for by **all 22 expert down-projections at 2 bits**, 0.94 G parameters at the cheapest width. The 4-bit map breached no floor at all; the 3-bit map breached 15, the tied embedding 8→4 plus fourteen `gate_up` banks 4→2 or 3, which is the soft-floor mechanism doing what P4 specified instead of returning the floor map. Running the tool for the first time found two defects a transcribed table would have carried: `embedding` reads floor 8 rather than the 4 of `DEFAULT_FLOOR_BITS` because the tie to `lm_head` raises it, and `attn.o` is 24 modules because 18 of them are the short-conv block's `out_proj`. **But +19.13 credits DynQuant-the-method for what is so far only DynQuant-the-allocator.** Nothing in this panel separates the map's *structure* from the *signal that chose it*; the decomposing arm is a matched-byte mixed-width map allocated from shuffled scores, or rank-product, or uniform-within-role, and it has not been run on this model. The split is not portable from elsewhere — the signal was 12% of the margin on Qwen3.5-2B and 56% on Ministral-8B, and at 4 bits on Qwen a plain RTN map *tied* DynQuant at matched bytes | [`phase4-text2sql-mixture.md`](phase4-text2sql-mixture.md) §13.4 |
...
## 22. Phase 4 — nineteen points at 3 bits, and the control that would decompose them
...
**And here is what the row does not claim.** +19.13 is byte-matched, paired and enormous, and it
credits *DynQuant-the-method* for what this panel demonstrates about *DynQuant-the-allocator*: that
a mixed-width map holding routers at 8 bits and expert down-projections at 2 beats a uniform 3-bit
recipe by nineteen points. Nothing here separates the map's structure from the signal that chose
it. The decomposing arm is a matched-byte mixed-width map allocated *without* the signal — shuffled
scores, rank-product, or uniform-within-role at the same anchor — and it has not been run on this
model. The split is not portable from the other campaigns: the signal was 12% of the margin on
Qwen3.5-2B and 56% on Ministral-8B, and at 4 bits on Qwen a plain RTN map tied DynQuant at matched
bytes. A second control worth the same GPU time is GPTQ and AWQ handed *DynQuant's* bit map, which
would price the allocator against the quantizer. Until one lands, the defensible claim is that
role-aware mixed-width allocation beats uniform 3-bit by nineteen points on this model, and the
share attributable to plasticity-times-saliency is unmeasured here.
