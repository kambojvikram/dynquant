...
5. **The margin over naive rounding is 88 % architectural prior and 12 % training signal.**
   ...
   into **22.62 points of allocation and 3.16 of signal** (p = 1.9e−15). That constant is a
   within-role shuffle, which §22 shows names only the first rung of a longer ladder, so 3.16
   is a floor under the signal term and 12 % a floor under its share.
...
so the 12 % figure is scoped to its campaign, not retracted. Re-checked at §22: both shares
split `dq` − `rtn` at a within-role shuffle and neither campaign ran a signal-free allocator, so
12 % and 56 % are one-rung numbers and each is a lower bound. The 4.25 row is deliberately left
...
**The earlier campaigns' shares are one-rung numbers.** 12% on Qwen3.5-2B and 56% on Ministral-8B
both split `dq` − `rtn` into `dq` − `shuf` and `shuf` − `rtn`, and neither ran a
signal-free-allocator arm, so on this ladder their signal term is the first rung alone. On that
definition this campaign reads 4.0% — 0.77 against 18.36 — which extends the series and fits its
trend, the share growing as the allocator's structural advantage shrinks. The 49.6% is a different
quantity. Still open: `uniform` drops the ranking and the measured `dL` together, so the +8.71 does
